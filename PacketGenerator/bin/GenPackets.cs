...
class PlayerInfoReq : IPacket
{
	...
	public class Skill
	{
	    ...
	    public bool Write(Span<byte> s, ref ushort count)
	    {
			...
			count += sizeof(ushort);
			foreach (Attribute attribute in attributes) success &= attribute.Write(s, ref count);
    ...
    public ArraySegment<byte> Write()
    {
		...
		count += sizeof(long);
		ushort nameLen = (ushort)Encoding.Unicode.GetBytes(name, 0, name.Length, 
		...
		count += sizeof(ushort);
		foreach (Skill skill in skills) success &= skill.Write(s, ref count);
